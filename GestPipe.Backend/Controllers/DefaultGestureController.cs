...
public class DefaultGestureController : ControllerBase
{
    ...
    public async Task<ActionResult<List<DefaultGestureDto>>> GetDisplay()
    {
        ...
        {
            Id = g.Id,
            Name = typeDict.ContainsKey(g.GestureTypeId) ? typeDict[g.GestureTypeId] : new Dictionary<string, string>(),
            Type = typeNameDict.ContainsKey(g.GestureTypeId) ? typeNameDict[g.GestureTypeId] : new Dictionary<string, string>(),
            Accuracy = g.Accuracy,
            Status = g.Status,
