...
namespace GestPipePowerPonit.I18n
{
    ...
    public static class I18nHelper
    {
        public static string GetLang()
            => CultureManager.CurrentCultureCode.StartsWith("vi") ? "vi" : "en";

        public static string GetLocalized(Dictionary<string, string> dict, string fallback = "")
        {
            if (dict == null) return fallback;
            var lang = GetLang();
            if (dict.TryGetValue(lang, out var v)) return v;
            if (dict.TryGetValue("en", out var en)) return en;
            return dict.Values.FirstOrDefault() ?? fallback;
        }

        public static string GetString(string en, string vi)
        {
            var lang = CultureManager.CurrentCultureCode.StartsWith("vi") ? "vi" : "en";
