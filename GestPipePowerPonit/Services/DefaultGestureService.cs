...
{
    public class DefaultGestureService
    {
        private readonly HttpClient _httpClient;

        public DefaultGestureService()
        {
            _httpClient = new HttpClient();
            _httpClient.BaseAddress = new Uri("https://localhost:7219/");
        }

        public async Task<List<DefaultGestureDto>> GetDefaultGesturesAsync()
        {
        ...
        public string GetGestureDescription(GestureDetailsDto gesture)
        {
            ...
            string typeName = I18nHelper.GetLocalized(gesture.Type); // lấy string đúng lang từ dict
            string direction = "";
        ...
        public string GetInstructionTable(GestureDetailsDto gesture)
        {
            ...
            string typeName = I18nHelper.GetLocalized(gesture.Type); // Sửa ở đây!
            string direction = "";
            if (typeName == I18nHelper.GetString("Static", "Tĩnh"))
            {
        ...
        {
            return await _httpClient.GetFromJsonAsync<GestureDetailsDto>($"/api/defaultgesture/{id}/detail");
        }
    }
}